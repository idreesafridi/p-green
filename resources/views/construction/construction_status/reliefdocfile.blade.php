@if ($prnotdoc->state != 0 && $prnotdoc->file_name != null || $prnotdoc->folder_name != null)
@php
...
@endphp
            ...
            <td class="space">
                @if ($prnotdoc->file_name)
                    <form action="{{ route('replace_rel_doc_files') }}" method="POST" enctype="multipart/form-data" id="replace_rel_doc_files_form_{{ $prnotdoc->id }}">
                        ...
                        <input type="text" name="relief_doc_id" value="{{ $relief_doc_file->id }}" hidden>

                        <input type="text" name="bydefault" value="{{ $prnotdoc->bydefault }}" hidden>

                        <input type="text" name="relief_doc_f_name" value="{{ $relief_doc_file->folder_name }}" hidden>
                    ...
                    <div class="spinner-border d-none" role="status" id="replace_rel_doc_files_spinner_{{ $prnotdoc->id }}">
                ...
                @if (isset($count) && $count > 0 || $prnotdoc->file_name != null)
                    ...
                    @else
                        ...
                        <button type="button" class="btn btn-link btn-sm text-danger d-inline"
                            onclick="location.href='{{ route('delete_relief_folder', $prnotdoc->id) }}'">
        ...
        <x-file-preview-modal modelId="viewFileModal{{ $prnotdoc->id }}" filepath="{{ $prnotdoc->file_path }}">
            @slot('filename', $prnotdoc->file_name)
