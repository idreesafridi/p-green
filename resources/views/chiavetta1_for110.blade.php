@php
    $pr_not_doc_id = $data110['pr_not_doc_id']->pr_not_doc_id;
    $docname = 'Documenti 110';
    $parent_folder_name = 'Documenti SAL 110';
@endphp
<tr>
    ...
    <td class="space">
        <form action="{{ route('replace_sub2_file') }}" method="POST" enctype="multipart/form-data"
            id="replace_sub2_file_form_{{ $prnotdoc->id }}">
            @csrf
            ...
            @if ($prnotdoc->updated_by == null && $prnotdoc->file_name != null)
                <input type="file" autocomplete="off" class="form-control" id="" name="file"
                    style="color:grey !important;" onchange="replace_sub2_file('{{ $prnotdoc->id }}')">
            @endif
        </form>
        <div class="nav nav-tabs border-bottom-0 d-none" role="tablist"
            id="replace_sub2_file_spinner_{{ $prnotdoc->id }}">
