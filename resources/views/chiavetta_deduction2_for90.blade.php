@php
    $pr_not_doc_id = $data90['pr_not_doc_id']->pr_not_doc_id;
    $docname = 'Documenti 90';
    $parent_folder_name = 'Documenti SAL 90';
@endphp
<tr>
    ...
    <td class="space">
        <form action="{{ route('replace_sub2_file') }}" method="POST" enctype="multipart/form-data" id="replace_sub2_file_deduction2_form_{{ $prnotdoc->id }}">
            @csrf
            ...
            <input type="text" name="type_of_dedection_sub1_id" value="{{ $prnotdoc->type_of_dedection_sub1_id }}" hidden>
            ...
            @if ($prnotdoc->updated_by == null && $prnotdoc->file_name != null)
                <input type="file" autocomplete="off" class="form-control" id="" name="file"
                    style="color:grey !important;" onchange="replace_sub2_file_deduction2('{{ $prnotdoc->id }}')">
            @endif
        </form>
        <div class="nav nav-tabs border-bottom-0 d-none" role="tablist" id="replace_sub2_file_deduction2_spinner_{{ $prnotdoc->id }}">
