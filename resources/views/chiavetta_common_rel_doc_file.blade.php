    ...
    <tr>
        ...
        <td class="space">
            @if ($prnotdoc->file_name)
                <form action="{{ route('replace_rel_doc_files') }}" method="POST" enctype="multipart/form-data" id="replace_rel_doc_files_common_rel_doc_file_form_{{ $prnotdoc->id }}">
                    @csrf
                    ...
                    @if ($prnotdoc->updated_on == null)
                        <input type="file" autocomplete="off" class="form-control" id="" name="file"
                            style="color:grey !important;" onchange="replace_rel_doc_files_common_rel_doc_file('{{ $prnotdoc->id }}')">
                    @endif
                </form>
                <div class="nav nav-tabs border-bottom-0 d-none" role="tablist" id="replace_rel_doc_files_common_rel_doc_file_spinner_{{ $prnotdoc->id }}">
