<tr>
    <td>
        <button type="button" class="viewFileBtn" data-bs-toggle="modal"
            data-bs-target="#viewFileModal{{ $regpracdoc->id }}"
    ...
    <td class="space">
        <form action="{{ route('regprac_file_upload') }}" method="POST" enctype="multipart/form-data" id="regprac_file_upload_common506590_form_{{ $regpracdoc->id }}">
            @csrf
            ...
            @if ($regpracdoc->updated_by == null && $regpracdoc->bydefault != 0)
                <input type="file" autocomplete="off" class="form-control" id="" name="fileTest"
                    style="color:grey !important;" onchange="regprac_file_upload_common506590('{{ $regpracdoc->id }}')">
            @endif

        </form>
        <div class="nav nav-tabs border-bottom-0 d-none" role="tablist" id="regprac_file_upload_common506590_spinner_{{ $regpracdoc->id }}">
...
<!--  Modal -->
<x-reminder-email-model modelId="bell{{ $regpracdoc->id }}Modal"
    folderName="{{ $regpracdoc->folder_name == null ? $regpracdoc->file_name : $regpracdoc->folder_name }}"
    conId="{{ $construct_id }}" />
<!-- End Modal -->

<!-- Replace Document Modal -->
<div class="modal fade" id="replaceDocModal{{ $regpracdoc->id }}" aria-labelledby="exampleModalLabel" aria-modal="true"
    role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content send-email">
            ...
            @if ($regpracdoc->file_name)
                <form action="{{ route('regprac_file_upload') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        ...
                        <input type="text" name="status_regprac_id" value="{{ $regpracdoc->status_reg_prac_id }}"
                            hidden>
