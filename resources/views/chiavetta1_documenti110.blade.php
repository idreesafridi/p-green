<tr>
    ...
    <td class="space">
        <form action="{{ route('replace_sub1_file') }}" method="POST" enctype="multipart/form-data" id="replace_sub1_file_form_{{ $prnotdoc->id }}">
            @csrf
            ...
            @if ($prnotdoc->updated_by == null && $prnotdoc->file_name != null)
                <input type="file" autocomplete="off" class="form-control" id="" name="file"
                    style="color:grey !important;" onchange="replace_sub1_file('{{ $prnotdoc->id }}')">
            @endif

        </form>
        <div class="nav nav-tabs border-bottom-0 d-none" role="tablist" id="replace_sub1_file_spinner_{{ $prnotdoc->id }}">
