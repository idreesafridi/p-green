<tr>
    ...
    <td class="space">
        <form action="{{ route('replace_file') }}" method="POST" enctype="multipart/form-data" id="replace_file_prnodoc_file_form_{{ $prnotdocfile->id }}">
            @csrf
            ...
            @if ($prnotdocfile->updated_on == null)
                <input type="file" autocomplete="off" class="form-control" id="" name="file"
                    style="color:grey !important;" onchange="replace_file_prnodoc_file('{{ $prnotdocfile->id }}')">
            @endif
        </form>
        <div class="nav nav-tabs border-bottom-0 d-none" role="tablist" id="replace_file_prnodoc_file_spinner_{{ $prnotdocfile->id }}">
    ...
    <div style="display: inline-flex; width: 100%;">
        ...
        @if ($prnotdocfile->updated_on != null)
            <a class="btn btn-link btn-sm text-dark d-inline" href="{{ route('download_prenoti_file', $prnotdocfile->id) }}">
...
<!--  Modal -->
<x-reminder-email-model modelId="bell{{ $prnotdocfile->id }}Modal"
    folderName="{{ $prnotdocfile->folder_name == null ? $prnotdocfile->file_name : $prnotdocfile->folder_name }}"
    conId="{{ $construct_id }}" />
<!-- End Modal -->
