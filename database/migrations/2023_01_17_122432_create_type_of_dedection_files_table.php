...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('type_of_dedection_files', function (Blueprint $table) {
            // $table->UUID('id')->primary();

            // $table->foreignUuid('type_of_dedection_sub2_id')->references('id')->on('type_of_dedection_sub2s')->onDelete('cascade');

            // $table->foreignUuid('construction_site_id')->references('id')->on('construction_sites')->onDelete('cascade')->nullable();

            $table->id();

            $table->unsignedBigInteger('type_of_dedection_sub2_id')->nullable();
            $table->foreign('type_of_dedection_sub2_id')->references('id')->on('type_of_dedection_sub2s')->onDelete('cascade')->onUpdate('cascade');
    ...
    {
        Schema::dropIfExists('type_of_dedection_files');
    }
};
